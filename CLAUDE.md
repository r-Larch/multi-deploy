...
- **`bin/lib-app`** — shared library. The contract everything depends on: `load_app <name>` sources `apps/<name>/app.env` and sets `NAME`, `APP_META_DIR`, `APP_REPO_DIR`, `APP_SSH_DIR`, `APP_COMPOSE_FILE`, and exports `GIT_SSH_COMMAND` (per-app SSH key with fallback to `/root/.ssh`). `build_compose_cmd` assembles the `compose_cmd` array (`docker compose --project-name <NAME> -f <APP_COMPOSE_FILE>`).
